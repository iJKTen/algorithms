...
Let's assume that the length of the array is 10, and we will have two variables to keep track of where our array begins and end. Let's call these two variables, beginIndex and endIndex. When this function is called for the first time, beginIndex will be 0 and endIndex will be 9. The middle index of the array will be beginIndex + (endIndex - beginIndex) / 2 which will give us 4.

Check, if the value at index 4 is the element we are looking for and if it is then we return that index, in this case 4.

If the value at index > 4 the value we are looking for, then we must assume that the value is present between index 0 and index mid. So now the beginIndex will be 0 and endIndex will be mid.

If the value at index < 4 the value we are looking for, then we must assume that the value is present between index mid and index length of the array - 1. So nowthe beginIndex will be mid and endIndex will be length of the array - 1.

We will not be splitting the array, we will keep track of where the array starts and where it ends, using two variables, beginIndex and endIndex. We will change these two variables by comparing x with the middle element of the array.

If you array has even number of elements then you will get a whole number, otherwise you get a decimal number, which is not an index in the array and it won't serve our puropse. You, get a whole number by passing the number to Math.floor (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/floor) which will give you a whole number. This number we will call mid and it is an index present in our array. Keep track of this number.
...
endIndex will be mid - 1, you subtract - 1 because the number is not present at index mid and remember, array starts at index 0, meaning, an array with 10 elements, will have index from 0 - 9.
